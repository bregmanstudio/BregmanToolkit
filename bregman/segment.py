...
class TimeSpan(object):
    def __init__(self, start_time=None, end_time=None, duration=None):
        ...
        self.duration = float(
            end_time) - self.start_time if duration is None else float(duration)
        self.end_time = self.start_time + \
            float(duration) if end_time is None else float(end_time)
...
class GeneralAudioSegmentor(Segmentor):
    ...
    def extract(self, media, num_clusters, feature=features.LogFrequencyCepstrum, filter=True, **kwargs):
        ...
        for i in range(len(self.diffs) - 1):
            self.segmentation.append(Segment(self.diffs[
                                     i] / float(self.frame_rate), self.diffs[i + 1] / float(self.frame_rate), label=seg_labels[i]))
    ...
    def cluster_plot(self, nofig=False):
        ...
        for k in range(self.num_clusters):
            z[k, np.where(self.assigns == k)[0]] = k + 1
        ...
        for k in range(self.num_clusters):
            ...
            for j in self.diffs[kpos]:
                for l in self.diffs[kpos]:
                    arc = pl.Circle(((l + j) / 2.0, 0.), radius=abs(l - j) / 2.0, color=cols[
                                    k % len(cols)], fill=0, linewidth=linewidth, alpha=alpha)
                    ax.add_patch(arc)
        pl.show()
