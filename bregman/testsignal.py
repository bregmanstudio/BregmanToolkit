...
def _check_signal_params(**params):
    ...
    params['phase_offset'] = float(
        params.get('phase_offset', p['phase_offset']))
    params['num_harmonics'] = int(
        params.get('num_harmonics', p['num_harmonics']))
...
def sinusoid(**params):
    ...
    x = pylab.sin(
        TWO_PI * params['f0'] / params['sr'] * t + params['phase_offset'])
    return x
...
def noise(noise_fun=pylab.rand, **params):
    ...
    [b, a] = scipy.signal.filter_design.butter(
        4, bw * 2 * pylab.pi / sr, btype='low', analog=0, output='ba')
    g = scipy.signal.lfilter(b, a, g)
    # Phase modulation with *filtered* noise (side-band modulation should be
    # narrow-band at bw)
...
def _check_rhythm_params(signal_params, rhythm_params, patterns):
    ...
    if not num_timbres:
        raise TestSignalError(
            "rhythm: signal_params lists and pattern n-tuple lengths don't match")
    return num_timbres
...
def rhythm(signal_params=None, rhythm_params=None, patterns=None):
    ...
    for beat in range(num_beats):
        for p, pat in enumerate(patterns):
            if (pat & (test_bit >> beat)):
                snd_ptr[p] = 0

        for t in range(num_timbres):
            idx = pylab.array(
                pylab.r_[snd_ptr[t]:snd_ptr[t] + sxt_dur * dt], dtype='int')
