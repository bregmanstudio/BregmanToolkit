...
class WavOpen:
    ...
    def open(self, filename, n, verbosity=0):
        if self.sound is None:
            ...
            if verbosity:
                print('num_frames=%i, num_secs=%f' %
                      (self.num_frames, self.num_frames / self.sample_rate))
    ...
    def get_all_frames(self):
        ...
        if rawdata:
            signal = wave.struct.unpack(
                '%ih' % self.num_frames * self.num_channels, rawdata)  # transform to signal
            del rawdata
    ...
    def next(self):
        ...
        if num_to_read > 0:
            ...
            if rawdata:
                signal = wave.struct.unpack(
                    '%ih' % (num_to_read * self.num_channels), rawdata)  # transform to signal
                ...
                raise Exception(
                    "Could not read sufficient sound data at index %d" % self.index)
        else:
            raise StopIteration
...
if os.name == 'posix':
    ...
    def play_snd(data, sample_rate=44100):
        ...
        _wav_write(data, AUDIO_TMP_FILE, sample_rate)
        winsound.PlaySound(
            AUDIO_TMP_FILE, winsound.SND_FILENAME | winsound.SND_ASYNC)
