...
class PLCA(object):
    ...
    def analyze(cls,
        ...
        if Wcomponents is None:
            Wcomponents = np.arange(rank)
    ...
    def compute_logprob(self, W, Z, H, recon):
        ...
        logprob += (np.sum((self.alphaW - 1) * np.log(W + EPS * W)) + np.sum(
            (self.alphaZ - 1) * np.log(Z + EPS * Z)) + np.sum(
                (self.alphaH - 1) * np.log(H + EPS * H)))
        # Add Entropic priors.
    ...
    @staticmethod
    def _apply_entropic_prior_and_normalize(param,
                                            evidence,
                                            beta,
                                            nu=50,
                                            niter=30,
...
class SIPLCA(PLCA):
    ...
    def plot(self, V, W, Z, H, curriter=-1):
        ...
        plottools.plotall(
            [V, WZH] + [
                self.reconstruct(
                    W[:, z, :], Z[z], H[z, :], circular=self.circular)
        ...
        wxticks = [[]] * (
            3 * nrows + rank + 1) + [list(range(0, W.shape[2], 10))]
        ...
        plottools.plotall(
            ...
            cmap=plt.cm.hot,
            ylabel=r'$*$',
            xticks=[[]] * (3 * nrows - 1) + [list(range(0, V.shape[1], 100))])
        plt.draw()
    ...
    def do_mstep(self, curriter):
        Zevidence = self._fix_negative_values(
            self.VRW.sum(2).sum(0) + self.alphaZ - 1)
        initialZ = normalize(Zevidence)
        Z = self._apply_entropic_prior_and_normalize(
            initialZ, Zevidence, self.betaZ, nu=self.nu)
...
class SIPLCA2(SIPLCA):
    ...
    def do_estep(self, W, Z, H):
        WZH = self.reconstruct(
            W, Z, H, circular=[self.circularF, self.circularT])
        logprob = self.compute_logprob(W, Z, H, WZH)
        ...
        for r in range(self.winF):
            ...
            for tau in range(self.winT):
                ...
                tmp = ((WZshifted[:, :, tau][:, :, np.newaxis] * Hshifted[
                    np.newaxis, :, :]).transpose((0, 2, 1)) * VdivWZH)
...
class FactoredSIPLCA2(SIPLCA2):
    ...
    def do_mstep(self, curriter):
        ...
        # P(\tau | k)
        Ptau_evidence = self._fix_negative_values(
            self.VRW.sum(0) + self.alphaT - 1)
        ...
        Pt_evidence = self._fix_negative_values(
            self.VRH.sum(2).T + self.alphaH - 1)
...
class DiscreteWSIPLCA2(FactoredSIPLCA2):
    ...
    def initialize(self):
        ...
        H = normalize(
            np.random.rand(self.rank, self.winF, self.nwarp, self.T),
            axis=[1, 2, 3])
        return W, Z, H
            ...
            for n, warp in enumerate(self.warpfactors):
                for delay, tau in enumerate(self.taus[n]):
                    ...
                    tmp = ((WZshifted[:, :, tau][:, :, np.newaxis] * Hshifted[
                        np.newaxis, :, :]).transpose((0, 2, 1)) * VdivWZH)
                    self.VRW[:, :, tau] += shift(
                        tmp.sum(1), -r, 0, self.circularF)
                    self.VRH[:, :, r, n] += shift(
                        tmp.sum(0), -delay, 0, self.circularT)
